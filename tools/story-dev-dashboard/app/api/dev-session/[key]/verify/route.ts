...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      action: 'verify',
      jobId,
      message: 'Verify workflow triggered',
      // Stream endpoint for real-time updates
      streamUrl: `/api/dev-session/${key}/stream?action=verify`,
    });
  } catch (error) {
    console.error('Error triggering verify workflow:', error);
    return NextResponse.json(
