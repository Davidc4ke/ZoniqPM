...
export async function POST(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      action: 'complete',
      jobId,
      message: 'Complete workflow triggered',
      // Stream endpoint for real-time updates
      streamUrl: `/api/dev-session/${key}/stream?action=complete`,
    });
  } catch (error) {
    console.error('Error triggering complete workflow:', error);
    return NextResponse.json(
