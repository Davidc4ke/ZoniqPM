...
function EditAppForm({ app, onClose }: { app: App; onClose: () => void }) {
  ...
  const handleClose = useCallback(() => {
    if (!updateApp.isPending) onClose()
  }, [updateApp.isPending, onClose])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    updateApp.mutate(
      { name, description: description || null, version, status },
      {
        onSuccess: () => {
          onClose()
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-xl border border-[#E8E4E0] bg-white p-6 shadow-xl">
        <h2 id="edit-app-title" className="mb-4 text-xl font-bold text-[#2D1810]">
          Edit App
        </h2>
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4">
            <div>
              <label htmlFor="edit-app-name" className="mb-1.5 block text-sm font-medium text-[#2D1810]">
            ...
            <div>
              <label htmlFor="edit-app-description" className="mb-1.5 block text-sm font-medium text-[#2D1810]">
                Description
              </label>
              <textarea
            ...
            <div>
              <label htmlFor="edit-app-version" className="mb-1.5 block text-sm font-medium text-[#2D1810]">
                Version
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="edit-app-status" className="mb-1.5 block text-sm font-medium text-[#2D1810]">
                Status
              </label>
              <select
                id="edit-app-status"
...
export function EditAppDialog({ app, isOpen, onClose }: EditAppDialogProps) {
  if (!isOpen) return null
  return <EditAppForm key={app.id} app={app} onClose={onClose} />
}
