...
interface FeatureFormProps {
  defaultValues?: Partial<CreateFeatureInput>
...
export function FeatureForm({ defaultValues, onSubmit, isPending, submitLabel, onCancel }: FeatureFormProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        <label htmlFor="feature-name" className="block text-sm font-medium text-[#2D1810]">
      ...
      <div>
        <label htmlFor="feature-description" className="block text-sm font-medium text-[#2D1810]">
          Description
        </label>
        <textarea
          ...
          placeholder="Brief description of this feature's purpose"
        />
        {errors.description && (
